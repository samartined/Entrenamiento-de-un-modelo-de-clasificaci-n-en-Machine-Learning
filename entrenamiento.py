...
for i in range(len(classes)):
    plt.scatter(
        dataframe.loc[dataframe["especies"] == classes[i], "longitud_sepalo"],
...
y_test_pred = knn_classifier(
    train.drop("especies", axis=1), train["especies"], test.drop("especies", axis=1), 5
)
...
new_data = pd.DataFrame(
    {
        "longitud_sepalo": [5.1, 4.9],
        "ancho_sepalo": [3.5, 3.0],
        "longitud_petalo": [1.4, 1.4],
